...
@Component({
  ...
  styleUrls: ['./admin-regay-kar-user.component.scss']
})
export class AdminRegayKarUserComponent {
  usersData: any;
  ...
  addUser(): void {
    if (this.UserForm.invalid) {
      ...
      this.spinner.show();
      const user_id = this.adminCookiesService.getCookie('AdminUser')?.id;
      if (!user_id) {
        return;
      }
      this.UserForm.value.created_by = user_id;
          ...
          if (response && response.status) {
            ...
            if (response) {
              if (response.type == "username") {
                ...
                this.toaster.error(this.translate.instant('code_exiest'), this.translate.instant('user'));
              } else {
  ...
  onCodeInput(event: any): void {
    ...
    if (!value.startsWith(this.originalFirstDigit)) {
      value = this.originalFirstDigit + value.slice(4);
      input.value = value;
      this.UserEditForm.get('code')?.setValue(value);
    }
  }
  openEditDialog(data: any): void {
    if (this.server_options && this.server_options.length > 0) {
      this.originalFirstDigit = data.code?.substring(0, 2).toUpperCase() || '';
  ...
  EditUser(): void {
    ...
    if (this.UserEditForm.valid) {
      ...
      const data = {
        ...
        code: this.UserEditForm.value.code.toUpperCase()
      };
