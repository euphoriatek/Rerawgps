...
@Component({
  selector: 'app-regay-kar-user-view',
  templateUrl: './regay-kar-user-view.component.html',
  styleUrls: ['./regay-kar-user-view.component.scss']
})
export class RegayKarUserViewComponent {
  ...
  getUsers(){
    this.spinner.show();
    this.api.getUserList("filter").subscribe({
      next: (response: any) => {
        this.spinner.hide();
        if (response && response.status) {
